...
public class UploadSuccessDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
        FrameLayout view = (FrameLayout) LayoutInflater
                .from (getContext ()).inflate (R.layout.common_upload_success_dialog, null);

        Widget_MessageDialog messageDialog = view.findViewById (R.id.common_upload_success_dialog);

        if (mOnButtonClickListener != null) {
            messageDialog.setListener (new Widget_MessageDialog.OnClickListener () {
                @Override
                public void onCancel() {
                    mOnButtonClickListener.exit ();
                }

                @Override
                public void onConfirm() {
                    mOnButtonClickListener.checkOut ();
                }
            });
        }
